...
public static class WriteDataFiles
{
    public static void WriteStrengthToFile(string filename, float[] strength, IWindow window, int windowSize, AudioFrame frame)
    {
        ...
        try
        {
            ...
            content.AppendLine();
            content.AppendLine("#       x         x[Hz]       fft[x]      fft[x]{dB}  fft[x]{bit} ");
            for (int i = 0, iEnd = strength.Length; i < iEnd; ++i)
            {
                content.AppendFormat("{0,10:#####0}   {1,10:F2}   {2,10:F7}   {3,10:F2}   {4,10:F2}  ",
                    i, i * precision, strength[i], 10 * Math.Log10(strength[i] + 1.0e-100), Math.Log2(strength[i] + 1.0e-100));
                content.AppendLine();
            }
        }
        finally
